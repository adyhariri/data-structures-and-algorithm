...
const maxInArray = (arr) => {
  return arr.reduce( (acc, i) => Math.max(acc, i));
  // Solution code here...
};
...
const updateNumbers = (obj) => Object.entries(obj).map(item => item.join(': '));
...
const hasChildrenValues = (arr, character) => {
  let childArr = Object.values(arr);
  let childs = childArr.find((item) => item.name === character);
